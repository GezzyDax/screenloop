...
- `GET /api/v1/media`, `POST /api/v1/media/upload`, `DELETE /api/v1/media/{id}`. Clip rows carry `has_poster` and `playlist_count`, so the library list can show a still and say where a clip is used without a request per row.
...
- `PATCH /api/v1/media/{id}` (`media.manage`) with `{ "title", "description", "silent", "compressed", "starts_at", "expires_at" }` — `title` is required, the rest optional. Only a change to `silent` or `compressed` re-runs the profiles; renaming does not. `starts_at` and `expires_at` are the two edges of the airing window, in Unix seconds, `null` for "no edge on this side"; omitting a field leaves that edge alone. A window that closes before it opens is a `400`, checked against the result of the change rather than the payload, so moving only the start cannot invert a window whose end was set weeks ago. A published clip whose start is still ahead reports the display state `scheduled` and is kept off screens exactly like an expired one.
...
- `POST /api/v1/media/{id}/window` (`media.manage`) with `{ "starts_at", "expires_at" }` — set both edges of the airing window at once. Exists beside `PATCH` because dating a whole selection should not have to send every clip's title back, which would rewrite names from a list that may already be stale. Same refusals as `PATCH`: `403` outside the caller's zones, `400` for a window that closes before it opens.
